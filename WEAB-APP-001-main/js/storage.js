class StorageManager {
    ...
    async initializeStorage() {
        try {
            // Always try GitHub first if authenticated
            if (githubStorage.isAuthenticated()) {
                const githubData = await githubStorage.fetchSongsDataWithRetry();
                if (githubData) {
                    this.songs = githubData.songs;
                    ...
                    // If no data on GitHub, initialize with sample data
                    await this.initializeWithSampleData();
                }
            } else {
                console.warn('GitHub not authenticated. Please set up GitHub token.');
                // Initialize with sample data if not authenticated
                await this.initializeWithSampleData();
            }
            ...
            throw new Error('Failed to initialize storage. Please ensure GitHub is properly configured.');
        }
    }

    async saveSongs() {
        if (!githubStorage.isAuthenticated()) {
            throw new Error('GitHub authentication required to save songs');
        }
